...
if __name__ == "__main__":
    ...
    parser.add_argument("type", help="Choose with which method you'd like to \
        crack a hash", choices=["Dictionary", "dictionary", "Permutations",
                                "permutations"])

    parser.add_argument("type2", help="Do you want to hash from a file or\
                        input hashes?", choices=["List", "File"])

    parser.add_argument("--multiprocessing", type=int, default=1, help="Leave it \
                        empty for singlethread, put in the number of workers\
                        for multiprocessing")
    ...
    if args.type == "dictionary":
        if args.multiprocessing == 1:
            if args.type2 == "List":
                ...
                time_start()
                HashCrackerDictionary_SingleHashFile(args.filename,
                                                     "list_of_hashes.txt")
                time_end()
        else:
            if args.type2 == "List":
                time_start()
                HashCrackerDictionary(args.multiprocessing, args.filename,
                                      [args.hashes])
                time_end()
            else:
                time_start()
                HashCrackerDictionaryHashFile(args.multiprocessing,
                                              args.filename,
                                              "list_of_hashes.txt")
                time_end()
    else:
        if args.multiprocessing == 1:
            if args.type2 == "List":
                time_start()
                HashCrackerPermutations_single(args.alphabet, args.max_length,
                                               args.hashes)
                time_end()
            else:
                time_start()
                HashCrackerPermutations_SingleHashFile(args.alphabet,
                                                       args.max_length,
                                                       "list_of_hashes.txt")
                time_end()
        else:
            if args.type2 == "File":
                time_start()
                HashCrackerPermutations(args.multiprocessing, args.alphabet,
                                        args.max_length, [args.hashes])
                time_end()
            else:
                time_start()
                HashCrackerPermutations(args.multiprocessing, args.alphabet,
                                        args.max_length, "list_of_hashes.txt")
                time_end()
