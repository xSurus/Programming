...
def HashCrackerPermutationsFileHashes(workers, alphabet, max_length, hashfile,
                                      num_hashes):
    ...
    with open(hashfile, "r") as file:
        single = file.readlines()
        list_hashes = [y.strip() for y in single]
        checker = functools.partial(cracker_perm_wrapper,
                                    list_hashes[:num_hashes])
        for length in range(1, max_length+1):
...
def becnhmark_single():
    ...
    while num_hashes <= 5000:
        t1 = time_start()
        for i in range(10):
            HashCrackerPermutations_SingleHashFile(alphabet, max_length,
...
def becnhmark_multi(num_hashes_start, num_workers_start):
    ...
    while num_workers > 0:
        ...
        while num_hashes <= 5000:
            t1 = time_start()
            for i in range(10):
                HashCrackerPermutationsFileHashes(num_workers, alphabet,
                                                  max_length, hashfile_name,
                                                  num_hashes)
            t2 = time_end(t1)
            time_av = t2 / 10
            f.write(str(time_av) + "\n")
